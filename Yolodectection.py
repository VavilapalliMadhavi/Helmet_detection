# yoloDetection.py: Contains functions for object detection (detectObject and related helper functions) using YOLOv3. These functions handle detecting objects in images/videos and labeling them.


import numpy as np
import cv2 as cv

def detectObject(CNNnet, total_layer_names, image_height, image_width, image, name_colors, class_labels, indexno, Boundingboxes=None, confidence_value=None, class_ids=None, ids=None, detect=True):
    if detect:
        ...
        if Boundingboxes is None or confidence_value is None or ids is None or class_ids is None:
            raise Exception('[ERROR] unable to draw boxes.')
